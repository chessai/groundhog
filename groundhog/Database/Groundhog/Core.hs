...
class (MonadFail m, MonadIO m, ConnectionManager (Conn m), PersistBackendConn (Conn m)) => PersistBackend m where
  type Conn m
  getConnection :: m (Conn m)

instance (MonadFail m, MonadIO m, PersistBackendConn conn) => PersistBackend (ReaderT conn m) where
